  ...
  return (
    <div className="min-h-screen bg-mjg-bg-primary pb-20">
      ...
      <div className="bg-mjg-bg-secondary shadow-mjg border-b border-mjg-border">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center">
            <button
              onClick={onBack}
              className="mr-4 text-mjg-accent hover:text-mjg-accent/80 transition-colors"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {/* Quick Stats Section */}
        {userRegs.length > 0 && (
          <div className="mb-8 bg-mjg-card rounded-mjg shadow-mjg border border-mjg-border p-4">
            <h2 className="text-xl font-poppins font-semibold text-mjg-text-primary mb-4">
              Quick Stats
            </h2>
            <div className="space-y-2">
              <div className="flex justify-between items-center py-2 border-b border-mjg-border">
                <span className="text-mjg-text-primary font-semibold">My Classes This Week</span>
                <span className="text-2xl font-bold text-mjg-text-primary">{stats.classesThisWeek}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-mjg-border">
                <span className="text-mjg-text-primary font-semibold">My Classes This Month</span>
                <span className="text-2xl font-bold text-mjg-text-primary">{stats.classesThisMonth}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-mjg-text-primary font-semibold">Current Streak</span>
                <span className="text-2xl font-bold text-mjg-text-primary">
        ...
        {userRegs.length === 0 ? (
          <div className="text-center py-16 bg-mjg-card rounded-mjg shadow-mjg border border-mjg-border">
            ...
            <p className="text-xl font-semibold text-mjg-text-card">You haven't registered for any classes yet</p>
            <p className="text-mjg-text-secondary mt-2">Head back to find and register for workouts</p>
            <button
              onClick={onBack}
              className="mt-6 bg-mjg-accent text-white px-4 py-3 rounded-mjg font-semibold hover:shadow-mjg transition-all"
            >
              Browse Classes
            </button>
          </div>
          ...
          <div className="space-y-8">
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {upcomingRegs.map(reg => (
                    <div key={reg.id} className="bg-mjg-card rounded-mjg shadow-mjg border border-mjg-border p-4 hover:shadow-mjg-lg transition-all">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 bg-mjg-bg-secondary text-mjg-text-primary py-2 rounded-mjg text-sm font-semibold hover:bg-mjg-bg-secondary/70 transition-all"
                        >
                          Notes
                        </button>
                        <button
                          onClick={() => onCancelRegistration?.(reg.event.id)}
                          className="flex-1 bg-mjg-accent text-white py-2 rounded-mjg text-sm font-semibold hover:shadow-mjg transition-all"
                        >
                          Cancel
                        </button>
                      </div>
            ...
            {pastRegs.length > 0 && (
              <div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pastRegs.map(reg => (
                    <div key={reg.id} className="bg-mjg-card rounded-mjg shadow-mjg border border-mjg-border p-4 opacity-75">
                      <h3 className="font-bold text-lg text-mjg-text-secondary mb-2">
                        {reg.event.title}
                      </h3>
                      <p className="text-sm text-mjg-text-secondary">
                        {new Date(reg.event.date + 'T12:00:00').toLocaleDateString('en-US', { 
                      ...
                      <button
                        onClick={() => onSelectEvent(reg.event)}
                        className="mt-3 w-full bg-mjg-bg-secondary text-mjg-text-primary py-2 rounded-mjg text-sm font-semibold hover:bg-mjg-bg-secondary transition-all"
                      >
                        View Notes
                      </button>
                    </div>
