...
const ATTENDED = (record) => record?.attended !== false;
...
export function getAttendanceDate(record, events = []) {
...
export function calculateStreak(userId, weekStartDate, attendance = [], events = []) {
  if (!userId) return 0;
  ...
  attendance.forEach((record) => {
    ...
    const date = getAttendanceDate(record, events);
    if (!date) return;
...
export function withMemberStreaks(members = [], weekStartDate, attendance = [], events = []) {
  return members
    .map((member) => {
      const streak = member.is_drop_in || !member.user_id
        ? 0
        : calculateStreak(member.user_id, weekStartDate, attendance, events);
